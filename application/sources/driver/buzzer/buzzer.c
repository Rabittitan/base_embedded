...
void BUZZER_Init(void) {
  ...
  RCC_APB2PeriphClockCmd(BUZZER_IO_CLOCK, ENABLE); // <-- THÊM DÒNG NÀY
  ...
  TIM_TimeBaseStructure.TIM_Prescaler =
      (SystemCoreClock / 4000000) - 1; // <-- SỬA: Thêm -1
  ...
  // Disable buzzer (set pin to analog input to save power)
  GPIO_InitStructure.GPIO_Pin = BUZZER_IO_PIN;
...
void BUZZER_Silent(bool isSilent) { _buzzer_silent = isSilent; }
